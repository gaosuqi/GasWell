...
from gas_tft.layers.add_and_norm import AddAndNorm
from gas_tft.layers.gated_linear_unit import GLU
from gas_tft.layers.gated_residual_network import GatedResidualNetwork
from gas_tft.layers.interpretable_multi_head_attention import InterpretableMultiHeadAttention
from gas_tft.layers.linear_layer import LinearLayer
from gas_tft.layers.lstm_combine_and_mask import LSTMCombineAndMask
from gas_tft.layers.static_combine_and_mask import StaticCombineAndMask
...
class TFT(nn.Module):
    def __init__(self, raw_params):
        ...
        # self.num_inputs = len(self._known_regular_input_idx) + len(self._known_categorical_input_idx) +
        # self.output_size self.num_inputs = len(self._known_regular_input_idx) + self.output_size
        # self.num_inputs_decoder = len(self._known_regular_input_idx) self.num_inputs = len(
        # self._known_regular_input_idx) + len(self._known_categorical_input_idx) + self.output_size
    ...
    def get_tft_embeddings(self, all_inputs):
        ...
        regular_inputs, categorical_inputs \
            = all_inputs[:, :, :num_regular_variables], all_inputs[:, :, num_regular_variables:]
    ...
    def get_hyperparm_choices(cls):
        ...
        return {
            ...
            'max_gradient_norm': [0.01, 1.0, 10.0, 100.0],
            'num_heads': [1, 4],
            'stack_size': [1],
        }
